...
function EditAddress({onClose,fetchDeliveryAddress,editId}) {
    //  console.log(editId)
...
let fetchSpecificIdData=async()=>{
    try{
let res=await fetch(SummeryApi.GetAddress.url+'/'+`${editId}`,{
...
console.log('date not come',e.message)
    }
}
...
let EditDeliveryAddess = async (e) => {
 try {
   e.preventDefault();
//  console.log(Useradd)
   let res = await fetch(SummeryApi.EditAddress.url+'/'+`${editId}`, {
     method: SummeryApi.EditAddress.method,
     headers: {
 ...
 return (
   <div className=' w-[100vw] h-[100vh] bg-slate-200  absolute top-[0vw] left-[0vw] bg-opacity-50 transition-all'>
     <div className='bg-white rounded w-[65vw] md:w-[50vw]    absolute top-[23vw] md:top-[10vw]   left-[15vw] z-10 overflow-y-auto' style={{backgroundColor:'white !important'}}>
         ...
         <div className='flex justify-center w-[100%]'>  <h1 className='text-2xl font-semibold mt-6'>Edit address</h1> </div> 
...
<button className='text-2xl' onClick={onClose}><RxCross2/></button>
           ...
           <form action=""   className='mx-[10%]'>
             <div className='flex flex-col items-start  '> 
               ...
               <button type='submit' className='px-4 w-[100%] py-1.5 bg-blue-500 text-white self-center rounded-full my-3 text-center mb-[6vw]' onClick={EditDeliveryAddess}  >Update</button> 
           </form>
